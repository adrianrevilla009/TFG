...
function bps_print_filter ($f)
{
	...
	case '':
		if (isset ($values))
			return sprintf (esc_html__(' %1$s', 'bp-profile-search'), $values[0]);
		return sprintf (esc_html__(' %1$s', 'bp-profile-search'), $f->value);

	case 'contains':
		return sprintf (esc_html__('contiene: %1$s', 'bp-profile-search'), $f->value);

	case 'like':
		return sprintf (esc_html__('es como: %1$s', 'bp-profile-search'), $f->value);

	case 'one_of':
		if (count ($values) == 1)
			return sprintf (esc_html__('es: %1$s', 'bp-profile-search'), $values[0]);
		return sprintf (esc_html__('es uno de: %1$s', 'bp-profile-search'), implode (', ', $values));

	case 'match_any':
		if (count ($values) == 1)
			return sprintf (esc_html__('coincide: %1$s', 'bp-profile-search'), $values[0]);
		return sprintf (esc_html__('coincide con cualquiera: %1$s', 'bp-profile-search'), implode (', ', $values));

	case 'match_all':
		if (count ($values) == 1)
			return sprintf (esc_html__('coincide: %1$s', 'bp-profile-search'), $values[0]);
		return sprintf (esc_html__('coincide con todo: %1$s', 'bp-profile-search'), implode (', ', $values));

	case 'distance':
		if ($f->value['units'] == 'km')
			return sprintf (esc_html__('está dentro de: %1$s km of %2$s', 'bp-profile-search'), $f->value['distance'], $f->value['location']);
		return sprintf (esc_html__('está dentro de: %1$s miles of %2$s', 'bp-profile-search'), $f->value['distance'], $f->value['location']);
